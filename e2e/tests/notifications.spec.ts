...
const mockNotifications = [
  {
    ...
    kind: "comment",
    subjectUri:
      "at://did:plc:jh6n3ntljfhhtr4jbvrm3k5b/org.rwell.test.occurrence/seed1",
...
async function mockNotificationRoutes(
  ...
  await page.route(
    (url) =>
      url.pathname.endsWith("/api/notifications") &&
...
authTest.describe("Notifications", () => {
  authTest("sidebar shows unread badge", async ({
  ...
  authTest("sidebar hides badge when no unread", async ({
    authenticatedPage: page,
  }) => {
  ...
  authTest("clicking notifications link navigates to /notifications", async ({
    authenticatedPage: page,
  }) => {
    await mockNotificationRoutes(page);
    await page.goto("/");
  ...
  authTest("notifications page shows all notification types", async ({
    authenticatedPage: page,
  }) => {
    await mockNotificationRoutes(page);
    ...
    await authExpect(
      page.getByText("commented on your observation"),
    ).toBeVisible({ timeout: 10_000 });
    await authExpect(
      page.getByText("identified your observation"),
    ).toBeVisible();
    await authExpect(
      page.getByText("liked your observation"),
    ).toBeVisible();
  });

  authTest("notifications page shows actor handles", async ({
    authenticatedPage: page,
  }) => {
    await mockNotificationRoutes(page);
    await page.goto("/notifications");
    ...
    await authExpect(
      page.getByText("@alice.bsky.social").first(),
    ).toBeVisible({ timeout: 10_000 });
    await authExpect(page.getByText("@bob.bsky.social")).toBeVisible();
  });

  authTest("unread notifications have highlighted background", async ({
    authenticatedPage: page,
  }) => {
    await mockNotificationRoutes(page);
    await page.goto("/notifications");
    ...
    await authExpect(thirdItem).toHaveCSS(
      "background-color",
      "rgba(0, 0, 0, 0)",
    );
  });

  authTest("mark all read button works", async ({
    authenticatedPage: page,
  }) => {
    await mockNotificationRoutes(page);
    await page.goto("/notifications");
  ...
  authTest("clicking a notification navigates to the observation", async ({
    authenticatedPage: page,
  }) => {
    await mockNotificationRoutes(page);
    await page.goto("/notifications");
  ...
  authTest("empty state shows message", async ({
    authenticatedPage: page,
  }) => {
    await mockNotificationRoutes(page, {
      unreadCount: 0,
