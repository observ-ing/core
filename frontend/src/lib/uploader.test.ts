...
function createValidOccurrence(
  overrides: Partial<Parameters<OccurrenceUploader["upload"]>[0]> = {},
) {
  return {
...
describe("OccurrenceUploader", () => {
  ...
  describe("validateOccurrence", () => {
    ...
    describe("location validation", () => {
      ...
      it("throws if longitude is below -180", async () => {
        ...
        await expect(uploader.upload(data)).rejects.toThrow(
  ...
  describe("extractExif", () => {
    ...
    it("returns dateTime from lastModified when no EXIF", async () => {
      ...
      const jpegNoExif = createMockFileWithBytes(
    ...
    it("handles JPEG with non-EXIF marker before EXIF", async () => {
      ...
      const jpegWithApp0 = createMockFileWithBytes(
        [
          ...
          0xe0, // APP0 marker
          0x00,
          0x04, // length = 4 (including length bytes)
          0x00,
          0x00, // data
          0xff,
          0xe1, // EXIF marker
