...
async fn migrate_collection(
    ...
    let query = format!(
        "SELECT uri, cid, did FROM {table} WHERE uri LIKE '%/{old_collection}/%'"
    );
    ...
    for (did, records) in &by_did {
        info!("[{did}] Migrating {} {old_collection} records...", records.len());

        let agent = match restore_agent(oauth_client, did).await {
            Ok(a) => a,
            Err(e) => {
                error!("[{did}] Failed to restore session: {e} — skipping {} records", records.len());
                skipped += records.len();
                continue;
            }
        };
        ...
        for row in records {
            ...
            let update_query =
                format!("UPDATE {table} SET uri = $1, cid = $2 WHERE uri = $3");
...
async fn update_subject_uris(pool: &PgPool, dry_run: bool) {
    ...
    for table in SUBJECT_URI_TABLES {
        ...
        info!(
            "{table}: {} subject_uri references to update",
            count.0
        );
        ...
        match sqlx::query(&update_query).execute(pool).await {
            Ok(result) => {
                info!("  Updated {} subject_uri rows in {table}", result.rows_affected());
            }
            Err(e) => {
...
async fn main() {
    ...
    for &(old_collection, new_collection) in COLLECTION_MAP {
        ...
        let (m, s, f) =
            migrate_collection(&pool, &oauth_client, old_collection, new_collection, table, dry_run)
                .await;
        total_migrated += m;
        total_skipped += s;
        total_failed += f;
    ...
    let should_update_subjects = collection_filter
        .as_ref()
        .map_or(true, |f| f.contains(&"occurrence"));
