...
pub async fn get_profile_feed(
    ...
    let did =
        Did::new_owned(&did).map_err(|e| AppError::BadRequest(format!("Invalid DID: {e}")))?;
    ...
    let result = observing_db::feeds::get_profile_feed(&state.pool, did.as_str(), &options).await?;
