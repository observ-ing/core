...
import { validateObservation } from "../src/generated/types.js";
const COLLECTION = "net.inat.observation";
...
async function publishObservation(agent, observation) {
    ...
    console.log(`  Location: ${observation.location.decimalLatitude}, ${observation.location.decimalLongitude}`);
    const record = {
        $type: COLLECTION,
        ...observation,
    };
...
async function main() {
    ...
    const observation = {
        ...
        notes: "Beautiful orange California Poppy blooming along the hiking trail. Multiple individuals observed.",
        createdAt: new Date().toISOString(),
    };
    const result = await publishObservation(agent, observation);
    console.log("\nTo view this record, use:");
    console.log(`  curl "${agent.service}/xrpc/com.atproto.repo.getRecord?repo=${agent.session.did}&collection=${COLLECTION}&rkey=${result.uri.split("/").pop()}"`);
}
main().catch((err) => {
...
//# sourceMappingURL=publish-observation.js.map