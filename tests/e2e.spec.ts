...
authTest.describe("E2E CRUD flow", () => {
  ...
  authTest.fixme(
    "create, identify, and delete an occurrence",
    async ({ authenticatedPage: page }) => {
      ...
      await authTest.step("add identification", async () => {
        ...
        const idDone = page.waitForResponse(
          (resp) =>
            resp.request().method() === "POST" && resp.url().includes("/api/identifications"),
          { timeout: 15_000 },
        );
      ...
      await authTest.step("delete occurrence", async () => {
        ...
        const deleteDone = page.waitForResponse(
          (resp) =>
            resp.request().method() === "DELETE" && resp.url().includes("/api/occurrences/"),
          { timeout: 15_000 },
        );
